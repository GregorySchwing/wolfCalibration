...
# Trappe ETOH
FF_file_ethanol = 'files/trappe-ua.xml'
ethanol = mb.load('files/ethanol.mol2')
ethanol.name = 'ETO'
...
for r in range(0, numReplicates, 1):
    ...
    for x in range(0, len(LambdaVDWList)):
        ...
        gomc_control.write_gomc_control_file(charmm, NVT_Prod_conf_name, 
                                            'NVT', RunSteps=NumProdRunSteps, 
                                            Temperature=Temp_in_K, 
                                            ff_psf_pdb_file_directory=ff_psf_pdb_file_directory_name, 
                                            Restart=False, 
                                            check_input_files_exist=False,
                                            input_variables_dict=input_variables_dict_NVT_Prod
                                            )
    ...
    for element in itertools.product(WolfMethods, WolfPotentials):
        ...
        for x in range(0, len(LambdaVDWList)):
            ...
            gomc_control.write_gomc_control_file(charmm, 
                                                NVT_Prod_conf_name, 'NVT', 
                                                RunSteps=NumProdRunSteps, 
                                                Temperature=Temp_in_K, 
                                                ff_psf_pdb_file_directory=ff_psf_pdb_file_directory_name, 
                                                Restart=False, 
                                                check_input_files_exist=False,
