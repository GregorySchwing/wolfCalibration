...
for solute_i in solute_name_list:
    ...
    Free_energy_calc_file_reading_name = f"avg_std_free_energy_data_waterTIP4P_{solute_i}.csv"
    ...
    Color_for_Plot_Data_calc = 'b' 		#black
    Color_for_Plot_Vrabec_calc =  'r'  #gray
    Color_for_Plot_NIST_exp =  'k'  #gray
    ...
    Free_energy_calc_data = pd.read_csv(Free_energy_calc_file_reading_name,  sep=',',
                                        header=0,
                                        #index_col=0,
                                        #usecols=[2,3,4]
                                        )
    ...
    print(Free_energy_calc_data_df)
    Temp_calc_data = Free_energy_calc_data_df.iloc[:, 2]
    print(Temp_calc_data)
    avg_free_energy_calc_free = Free_energy_calc_data_df.iloc[:, 3].values.tolist()
    std_dev_free_energy_calc_free = Free_energy_calc_data_df.iloc[:, 4].values.tolist()
    ...
    plt.errorbar(Temp_calc_data, avg_free_energy_calc_free, yerr=std_dev_free_energy_calc_free,
             color=Color_for_Plot_Data_calc,
             marker='D', linestyle=Calc_Linestyle, markersize=PointSizes, markeredgewidth=MarkeredgeWidth,
             linewidth=ConnectionLineSizes, label=avg_free_energy_calc_free_label, fillstyle='none',
             )
    plt.errorbar(Temp_Vrabec_calc_data, avg_free_energy_Vrabec_calc_free , yerr=std_dev_free_energy_Vrabec_calc_free,
                 color=Color_for_Plot_Vrabec_calc,
                 marker='s',  linestyle=Calc_Linestyle, markersize=PointSizes, markeredgewidth=MarkeredgeWidth,
                 linewidth=ConnectionLineSizes, fillstyle='none',
                 capsize=10,
                 label=avg_free_energy_Vrabec_calc_free_label
                 )
    plt.plot(Temp_NIST_exp_data, avg_free_energy_NIST_exp_free,
             color=Color_for_Plot_NIST_exp,
             marker=None, linestyle=Exp_Linestyle, markersize=PointSizes, markeredgewidth=MarkeredgeWidth,
             linewidth=ConnectionLineSizes, fillstyle='none',
    ...
    plt.xlim(T_min, T_max)  # set plot range on x axis
